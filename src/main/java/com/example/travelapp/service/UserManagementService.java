...
public class UserManagementService {
	...
	public boolean addUser(User u, Set<String> roleIds) {
		...
		audit.log(SecurityContext.getCurrentUser().getId(), "CREATE", "User", u.getId(),
		          "{\"action\":\"create_user\",\"userId\":\"" + u.getId() + "\"}");
		...
		audit.log(SecurityContext.getCurrentUser().getId(), "UPDATE", "User", u.getId(),
		          "{\"action\":\"assign_roles\",\"userId\":\"" + u.getId() + "\",\"roles\":\"" + roleIds + "\"}");
		return true;
	}
	...
	public boolean updateUser(User u, Set<String> roleIds) {
		...
		audit.log(SecurityContext.getCurrentUser().getId(), "UPDATE", "User", u.getId(),
		          "{\"action\":\"update_user\",\"userId\":\"" + u.getId() + "\"}");

		if (!deleteRoles(u.getId())) {
			return false;
		}
		audit.log(SecurityContext.getCurrentUser().getId(), "UPDATE", "User", u.getId(),
		          "{\"action\":\"clear_roles\",\"userId\":\"" + u.getId() + "\"}");
	...
	public boolean deleteUser(String id) {
		...
		audit.log(SecurityContext.getCurrentUser().getId(), "UPDATE", "User", id,
		          "{\"action\":\"clear_roles\",\"userId\":\"" + id + "\"}");

		if (!userDao.delete(id)) {
			return false;
		}
		audit.log(SecurityContext.getCurrentUser().getId(), "DELETE", "User", id,
	...
	private boolean deleteRoles(String userId) {
		...
		try (var conn = com.example.travelapp.config.DataSourceProvider.getDataSource().getConnection();
